<template>
    <div class="bg-white px-4 shadow-lg rounded-xl">
        <p><br></p>
        <AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigVendas')"
            :RotaExcel="route('get.Excel.ConfigVendas')" />
                    ...
                    <div v-for="(label, key) in {
                        ...
                        <span class="p-float-label">
                            <template v-if="key === 'status'">
                                <Dropdown class="w-full" v-model="form2[key]" :options="statusOption" optionLabel="name"
                                    dataKey="value" required />
                            </template>
                            <template v-else>
                                <InputText v-model="form2[key]" :id="key" type="text" class="w-full" required />
                            </template>
            ...
            <div class="text-sm text-gray-500 flex items-center">
                <div class="space-x-6 flex ml-4 grid grid-cols-6 gap-6 max-md:grid-cols-1">
                    ...
                    <div v-for="(label, key) in {
                        ...
                        <Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns'][key]" />
                        <span>{{ label }}</span>
                    </div>
        ...
        <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
            <div class="inline-block min-w-full py-2 align-middle">
                <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Vendas</b></h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                        <thead class=" mb-24">
                            <tr class="text-gray-500 font-bold select-none" @click="setParams">
                                ...
                                <th v-if="formColumns.columns.id_cliente" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group"
                                    @click="orderBy = { column: 'id_cliente', sorting: sortTable(sortVal.id_cliente) ? sortVal.id_cliente = 1 : sortVal.id_cliente = 0 }">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">ID Cliente</span>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th v-if="formColumns.columns.tipo" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group"
                                    @click="orderBy = { column: 'tipo', sorting: sortTable(sortVal.tipo) ? sortVal.tipo = 1 : sortVal.tipo = 0 }">
                                    <div class="flex">
                                ...
                                <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%;"
                                    v-if="$page.props.userPermissions.includes('edit.ConfigVendas') || $page.props.userPermissions.includes('delete.ConfigVendas')">
                                    Ações</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
                                ...
                                <td v-if="formColumns?.columns?.id_cliente"
                                    class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">{{ data?.id_cliente }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
                                    style="width: 5%;"
                                    v-if="$page.props.userPermissions.includes('edit.ConfigVendas') || $page.props.userPermissions.includes('delete.ConfigVendas')">
                                    <Actions :routeDel="route('delete.ConfigVendas', { id: data?.token })"
                                        :routeUpdate="route('form.update.ConfigVendas', { id: data?.token })" />
                                </td>
                            </tr>
...
const formColumns = useForm({
    route_of_list: "list.ConfigVendas",
    columns: {
        id_veiculo: validateColumnsVisibility("id_veiculo"),
        id_cliente: validateColumnsVisibility("id_cliente"),
...
const form2 = useForm({
    id_veiculo: { value: props.Filtros?.id_veiculo || null },
    id_cliente: { value: props.Filtros?.id_cliente || null },
    valor: { value: props.Filtros?.valor || null },
    tipo: { value: props.Filtros?.tipo || null },
...
function del() {
    ...
    selected.value = '';
    Inertia.post(route("deleteSelected.ConfigVendas", { id: valor.value }));
}
function delTodos() {
    Inertia.post(route("deletarTodos.ConfigVendas"));
}

function RestaurarTodos() {
    Inertia.post(route("RestaurarTodos.ConfigVendas"));
}
...
function FiltroAvancadoAplica() {
    ...
    !orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";
    submitForm.post(route("listP.ConfigVendas"), {
        replace: false,
        data,
        onSuccess: () => {
