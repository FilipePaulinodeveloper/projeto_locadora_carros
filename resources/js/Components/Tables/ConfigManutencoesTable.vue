<template>
    <div class="bg-white px-4 shadow-lg rounded-xl">
        <p><br></p>
        <AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigManutencoes')"
            :RotaExcel="route('get.Excel.ConfigManutencoes')" />
                ...
                <div class="flex ml-4  grid grid-cols-8 gap-8 max-md:grid-cols-1">
                    ...
                    <div>
                        <span class="p-float-label">
                            <InputText v-model="form2.id_veiculo" id="id_veiculo" type="text" class="w-full" required />
                            <label for="id_veiculo" class="text-sm">ID Veículo</label>
                        </span>
                    </div>

                    <div>
                        <span class="p-float-label">
                            <InputText v-model="form2.data_manutencao" id="data_manutencao" type="date" class="w-full"
                                required />
                            <label for="data_manutencao" class="text-sm">Data Manutenção</label>
                        </span>
                    </div>
                    ...
                    <div>
                        <span class="p-float-label">
                            <InputText v-model="form2.valor" id="valor" type="number" class="w-full" required />
        ...
        <SlideUpDown v-model="toggleFilter" :duration="300">
            ...
            <div class="text-sm text-gray-500 flex items-center">
                <div class="space-x-6 flex ml-4 grid grid-cols-6 gap-6 max-md:grid-cols-1">
                    ...
                    <div class="flex items-center space-x-2">
                        <Checkbox @change="toggleColumns" :binary="true"
                            v-model="formColumns['columns']['data_manutencao']" />
                        <span>Data Manutenção</span>
                    </div>

                    <div class="flex items-center space-x-2">
                        <Checkbox @change="toggleColumns" :binary="true"
                            v-model="formColumns['columns']['descricao']" />
                        <span>Descrição</span>
                    </div>
        ...
        <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
            <div class="inline-block min-w-full py-2 align-middle">
                <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Manutenções</b></h2>
                        ...
                        <thead class=" mb-24">
                            <tr class="text-gray-500 font-bold select-none" @click="setParams">
                                ...
                                <th v-if="formColumns.columns.id_veiculo" scope="col"
                                    ...
                                    @click="orderBy = { column: 'id_veiculo', sorting: sortTable(sortVal.id_veiculo) ? sortVal.id_veiculo = 1 : sortVal.id_veiculo = 0 }">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">ID Veículo</span>
                                ...
                                <th v-if="formColumns.columns.data_manutencao" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group"
                                    @click="orderBy = { column: 'data_manutencao', sorting: sortTable(sortVal.data_manutencao) ? sortVal.data_manutencao = 1 : sortVal.data_manutencao = 0 }">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">Data Manutenção</span>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th v-if="formColumns.columns.valor" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group"
                                    @click="orderBy = { column: 'valor', sorting: sortTable(sortVal.valor) ? sortVal.valor = 1 : sortVal.valor = 0 }">
                                    <div class="flex">
                                ...
                                <th v-if="formColumns.columns.tipo" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group"
                                    @click="orderBy = { column: 'valor', sorting: sortTable(sortVal.tipo) ? sortVal.tipo = 1 : sortVal.tipo = 0 }">
                                    <div class="flex">
                                ...
                                <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%;"
                                    v-if="$page.props.userPermissions.includes('edit.ConfigManutencoes') || $page.props.userPermissions.includes('delete.ConfigManutencoes')">
                                    Ações</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
                                ...
                                <td v-if="formColumns?.columns?.id_veiculo"
                                    ...
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">{{ data?.id_veiculo }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td v-if="formColumns?.columns?.data_manutencao"
                                    class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">{{ data?.data_manutencao }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td v-if="formColumns?.columns?.valor"
                                    ...
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">{{ data?.valor }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
                                    style="width: 5%;"
                                    v-if="$page.props.userPermissions.includes('edit.ConfigManutencoes') || $page.props.userPermissions.includes('delete.ConfigManutencoes')">
                                    <Actions :routeDel="route('delete.ConfigManutencoes', { id: data?.token })"
                                        :routeUpdate="route('form.update.ConfigManutencoes', { id: data?.token })" />
                                </td>
                            </tr>
...
const formColumns = useForm({
    route_of_list: "list.ConfigManutencoes",
    columns: {
        id_veiculo: validateColumnsVisibility("id_veiculo"),
        data_manutencao: validateColumnsVisibility("data_manutencao"),
...
const form2 = useForm({
    id_veiculo: props.Filtros?.id_veiculo || null,
    data_manutencao: props.Filtros?.data_manutencao || null,
...
function del() {
    ...
    selected.value = '';
    Inertia.post(route("deleteSelected.ConfigManutencoes", { id: valor.value }));
}
function delTodos() {
    Inertia.post(route("deletarTodos.ConfigManutencoes"));
}

function RestaurarTodos() {
    Inertia.post(route("RestaurarTodos.ConfigManutencoes"));
}
...
function FiltroAvancadoAplica() {
    ...
    !orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";
    submitForm.post(route("listP.ConfigManutencoes"), {
        replace: false,
        data,
        onSuccess: () => {
