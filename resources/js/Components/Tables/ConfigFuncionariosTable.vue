<template>
    <div class="bg-white px-4 shadow-lg rounded-xl">
        <p><br></p>
        <AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigFuncionarios')"
            :RotaExcel="route('get.Excel.ConfigFuncionarios')" />
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                        <thead class=" mb-24">
                            <tr class="text-gray-500 font-bold select-none" @click="setParams">
                                ...
                                <th v-if="formColumns.columns.salario" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group"
                                    @click="orderBy = { column: 'salario', sorting: sortTable(sortVal.salario) ? sortVal.salario = 1 : sortVal.salario = 0 }">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">Salário</span>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th v-if="formColumns.columns.cargo" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group"
                                    @click="orderBy = { column: 'cargo', sorting: sortTable(sortVal.cargo) ? sortVal.cargo = 1 : sortVal.cargo = 0 }">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">Cargo</span>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%;"
                                    v-if="$page.props.userPermissions.includes('edit.ConfigFuncionarios') || $page.props.userPermissions.includes('delete.ConfigFuncionarios')">
                                    Ações</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
                                ...
                                <td v-if="formColumns?.columns?.salario"
                                    class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">{{ data?.salario }}</div>
                                        </div>
                                    </div>
                                </td>

                                <td v-if="formColumns?.columns?.cargo"
                                    class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">{{ data?.cargo }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
                                    style="width: 5%;"
                                    v-if="$page.props.userPermissions.includes('edit.ConfigFuncionarios') || $page.props.userPermissions.includes('delete.ConfigFuncionarios')">
                                    <Actions :routeDel="route('delete.ConfigFuncionarios', { id: data?.token })"
                                        :routeUpdate="route('form.update.ConfigFuncionarios', { id: data?.token })" />
                                </td>
                            </tr>
...
const formColumns = useForm({
    route_of_list: "list.ConfigFuncionarios",
    columns: {
        nome: validateColumnsVisibility("nome"),
        cpf: validateColumnsVisibility("cpf"),
        salario: validateColumnsVisibility("salario"),
        cargo: validateColumnsVisibility("cargo"),
...
const form2 = useForm({
    ...
    salario: props.Filtros?.salario || null,
    cargo: props.Filtros?.cargo || null,
...
function del() {
    ...
    selected.value = '';
    Inertia.post(route("deleteSelected.ConfigFuncionarios", { id: valor.value }));
}
function delTodos() {
    Inertia.post(route("deletarTodos.ConfigFuncionarios"));
}

function RestaurarTodos() {
    Inertia.post(route("RestaurarTodos.ConfigFuncionarios"));
}
...
function FiltroAvancadoAplica() {
    ...
    !orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";
    submitForm.post(route("listP.ConfigFuncionarios"), {
        replace: false,
        data,
        onSuccess: () => {
