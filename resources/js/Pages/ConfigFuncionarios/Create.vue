  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      ...
      <div class="mt-10 grid grid-cols-1 gap-6 max-md:grid-cols-1">
        ...
        <div>
          <span class="p-float-label">
            <InputText v-model="form.salario" id="salario" type="text" class="w-full" required maxlength="50" />
        ...
        <div>
          <span class="p-float-label">
            <Dropdown class="w-full" v-model="form.cargo" :options="cargoOption" optionLabel="name" dataKey="value"
              required />
...
function submit() {
  ...
  submitForm.post(route("store.ConfigFuncionarios"), {
    preserveState: true,
    onError: (errors) => {
