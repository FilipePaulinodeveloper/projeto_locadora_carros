        ...
        <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
            ...
            <div class="mt-10 grid grid-cols-1 gap-6 max-md:grid-cols-1">
                ...
                <div>
                    ...
                    <span class="p-float-label">
                        <InputText v-model="form.cpf" id="cpf" type="text" class="w-full" required maxlength="15"
                ...
                <div>
                    <span class="p-float-label">
                        <InputText v-model="form.salario" id="salario" type="text" class="w-full" required
                            maxlength="50" />
                        <label for="nome" class="text-sm">Salario</label>
                    </span>
                </div>
                ...
                <div>
                    {{ form.cargo }}
                    <span class="p-float-label">
                        <Dropdown class="w-full" v-model="form.cargo" :options="cargoOption" optionLabel="name" optionValue="value" dataKey="value" required />
                        <label for="status" class="text-sm">Cargo</label>
                    </span>
                </div>
                ...
                <div>
                    <span class="p-float-label">
                        <Dropdown class="w-full" v-model="form.status" :options="statusOption" optionLabel="name" optionValue="value" dataKey="value" required />
            ...
            <div class="flex space-x-5 mt-8">
                ...
                <Link :href="route('list.ConfigFuncionarios')" as="button" type="button"
...
const form = useForm({
    token: $propsPage?.value.ConfigFuncionarios.token,

    nome: $propsPage?.value.ConfigFuncionarios.nome,

    cpf: $propsPage?.value.ConfigFuncionarios.cpf,

    salario: $propsPage?.value.ConfigFuncionarios.salario,

    cargo: String($propsPage?.value.ConfigFuncionarios.cargo),

    status: String($propsPage?.value.ConfigFuncionarios.status),

    created_at: $propsPage?.value.ConfigFuncionarios.created_at,
});
...
function submit() {
    ...
    submitForm.post(
        route("update.ConfigFuncionarios", { id: $propsPage.value?.ConfigFuncionarios.token }),
        {
            preserveState: true,
            onError: (errors) => {
