    ...
    <form @submit.prevent="submit">
        <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
            ...
            <div class="mt-10 grid grid-cols-1 gap-6 max-md:grid-cols-1">
                ...
                <div>
                    <span class="p-float-label">
                        <Dropdown class="w-full" v-model="form.id_veiculo" :options="veiculos" optionLabel="placa"
                            optionValue="id" dataKey="id" required />
                ...
                <div>
                    <span class="p-float-label">
                        <Dropdown class="w-full" v-model="form.id_cliente" :options="clientes" optionLabel="nome"
                            optionValue="id" dataKey="id" required />
                        <label for="id_cliente" class="text-sm">Cliente</label>
                    </span>
                ...
                <div>
                    <span class="p-float-label">
                        <Dropdown class="w-full" v-model="form.tipo" :options="tipoVenda" optionLabel="name"
                            optionValue="value" dataKey="value" required />
                        <label for="id_veiculo" class="text-sm">Tipo</label>
                    </span>
            ...
            <div class="flex space-x-5 mt-8">
                ...
                <Link :href="route('list.ConfigVendas')" as="button" type="button"
...
const { getVeiculos, getClientes } = getId();
...
const form = useForm({
    id_veiculo: $propsPage?.value.ConfigVendas?.id_veiculo,
    id_cliente: $propsPage?.value.ConfigVendas?.id_cliente,
    valor: $propsPage?.value.ConfigVendas?.valor,
    tipo: $propsPage?.value.ConfigVendas?.tipo,
    status:String($propsPage?.value.ConfigVendas?.status),
    status: String($propsPage?.value.ConfigVendas?.status),
});
...
function submit() {
    ...
    submitForm.post(
        route("update.ConfigVendas", { id: $propsPage.value?.ConfigVendas?.token }),
{
    preserveState: true,
        onError: (errors) => {
