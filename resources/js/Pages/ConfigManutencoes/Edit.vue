    ...
    <form @submit.prevent="submit">
        <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
            ...
            <div class="mt-10 grid grid-cols-1 gap-6 max-md:grid-cols-1">
                ...
                <div>
                    <span class="p-float-label">
                        <Dropdown class="w-full" v-model="form.tipo" :options="tipoManutencao" optionLabel="name" optionValue="value"
...
const form = useForm({
    id_veiculo: $propsPage?.value.ConfigManutencoes.id_veiculo,
    data_manutencao: $propsPage?.value.ConfigManutencoes.data_manutencao,
    descricao: $propsPage?.value.ConfigManutencoes.descricao,
    valor: $propsPage?.value.ConfigManutencoes.valor,
    tipo: $propsPage?.value.ConfigManutencoes.tipo,
    status: String( $propsPage?.value.ConfigManutencoes.status)
});
...
function submit() {
    ...
    submitForm.post(
        route("update.ConfigManutencoes", { id: $propsPage.value?.ConfigManutencoes.token }),
        {
            preserveState: true,
            onError: (errors) => {
